...
impl<T: Borrow<[Node]> + BorrowMut<[Node]> + Index<usize,Output=Node> + IndexMut<usize,Output=Node> + Clone + Debug> RankVector<T> {
            ...
            match target_zone {
                2 => {
                   if self.nodes[target].rank < self.nodes[self.median.1].rank {
                       self.sums[1] -= target_node.data;
                       self.square_sums[1] -= target_node.data.powi(2);
                   }
                   if self.nodes[target].rank > self.nodes[self.median.0].rank {
                       self.sums[2] -= target_node.data;
                       self.square_sums[1] -= target_node.data.powi(2);
                   }
               },
               1 => {
                   self.sums[0] -= target_node.data;
                   self.square_sums[0] -= target_node.data.powi(2);
               },
               3 => {
                   self.sums[3] -= target_node.data;
                   self.square_sums[3] -= target_node.data.powi(2);
               },
    ...
    fn mpop(&mut self, target: usize) -> (f64,f64) {
        ...
        if target_zone != 0 {
            ...
            (new_median,target_node.data)
        }
        else {
            (self.median(),target_node.data)
        }
    }
    ...
    pub fn establish_sums(&mut self) {
        ...
        self.sums = [0.;4];
        let (m_left,m_right) = self.median;
        let left_zone = self.crawl_left(self.left).skip(1).take(self.zones[1]).collect::<Vec<usize>>();
        let right_zone = self.crawl_right(self.right).skip(1).take(self.zones[3]).collect::<Vec<usize>>();
        let middle_left = self.crawl_left(self.median.0).take_while(|&i| self.nodes[i].zone == 2).collect::<Vec<usize>>();
        let middle_right = self.crawl_right(self.median.1).take_while(|&i| self.nodes[i].zone == 2).collect::<Vec<usize>>();

        eprintln!("Left zone:{:?}",left_zone);
        eprintln!("Left middle zone:{:?}",middle_left);
        eprintln!("Right middle zone:{:?}",middle_right);
        eprintln!("Right zone:{:?}",right_zone);

        for i in middle_left.into_iter() {
            self.sums[1] += self.nodes[i].data;
            self.square_sums[1] += self.nodes[i].data.powi(2);
        }
        for i in middle_right.into_iter() {
            self.sums[2] += self.nodes[i].data;
            self.square_sums[2] += self.nodes[i].data.powi(2);
        }
        for i in left_zone {
            self.sums[0] += self.nodes[i].data;
            self.square_sums[0] += self.nodes[i].data.powi(2);
        }
        for i in right_zone {
            self.sums[3] += self.nodes[i].data;
            self.square_sums[3] += self.nodes[i].data.powi(2);
        }
    ...
    pub fn contract_left(&mut self) {
        ...
        let d = self.nodes[self.left].data;
        self.sums[0] += d;
        self.sums[1] -= d;

        let ds = d.powi(2);
        self.square_sums[0] += ds;
        self.square_sums[1] -= ds;
    ...
    pub fn contract_right(&mut self) {
        ...
        let data = self.nodes[self.right].data;
        self.sums[3] += data;
        self.sums[2] -= data;

        let data_squared = data.powi(2);
        self.square_sums[3] += data_squared;
        self.square_sums[2] -= data_squared;
    ...
    pub fn expand_left(&mut self) {
        ...
        let data = self.nodes[self.left].data;
        self.sums[0] -= data;
        self.sums[1] += data;

        let data_squared = data.powi(2);
        self.square_sums[0] -= data_squared;
        self.square_sums[1] += data_squared;
    ...
    pub fn shift_median_left(&mut self) {
        match self.median.0 == self.median.1 {
            false => {
                self.sums[1] -= self.nodes[self.median.0].data;
                self.median = (self.nodes[self.median.1].previous,self.nodes[self.median.1].previous)
            },
            true => {
                self.sums[2] += self.nodes[self.median.1].data;
    ...
    //     }
    //
    //     let left_slow_sum: f64 = self.crawl_left(self.left).take(self.zones[1]).map(|i| self.nodes[i].data).sum();
    //     let right_slow_sum: f64 = self.crawl_right(self.right).take(self.zones[3]).map(|i| self.nodes[i].data).sum();
    //     let middle_slow_sum: f64 = self.left_to_right().into_iter().map(|i| self.nodes[i]).filter(|node| node.zone == 2).map(|node| node.data).sum();
    //
    //     if
    //         left_slow_sum - self.sums[0] > 0.00001 ||
    //         right_slow_sum - self.sums[3] > 0.00001 ||
    //         middle_slow_sum - (self.sums[1] + self.sums[2]) > 0.00001
    //     {
    //         println!("Left slow sum:{}, middle slow sum:{}, right slow sum:{}",left_slow_sum,middle_slow_sum,right_slow_sum);
    //         println!("Internal sums:{:?}",self.sums);
    //         println!("{:?}",self);
    //         panic!("Sum mismatch")
    //
    //     }
...
impl RankVector<Vec<Node>> {
    ...
    pub fn derive_stencil(&self, stencil: &Stencil) -> RankVector<Vec<Node>> {
        ...
        let mut new_sums = [0.;4];
        let mut new_square_sums = [0.;4];
    ...
    pub fn check_integrity(&self) {
        ...
        // let left_slow_sum: f64 = self.crawl_left(self.left).take(self.zones[1]).map(|i| self.nodes[i].data).sum();
        // let right_slow_sum: f64 = self.crawl_right(self.right).take(self.zones[3]).map(|i| self.nodes[i].data).sum();
        // let slow_middle_left: f64 = self.crawl_left(self.median.1).take_while(|&i| self.nodes[i].zone == 2).map(|i| self.nodes[i].data).sum();
        // let slow_middle_right: f64 = self.crawl_right(self.median.0).take_while(|&i| self.nodes[i].zone == 2).map(|i| self.nodes[i].data).sum();

        let left_slow_sum: f64 = self.left_to_right().into_iter().filter(|&i| self.nodes[i].zone == 1).map(|i| self.nodes[i].data).sum();
        let right_slow_sum: f64 = self.left_to_right().into_iter().filter(|&i| self.nodes[i].zone == 3).map(|i| self.nodes[i].data).sum();
        let slow_middle_left: f64 = self.left_to_right().into_iter().filter(|&i| self.nodes[i].zone == 2 && self.nodes[i].rank < self.nodes[self.median.1].rank).map(|i| self.nodes[i].data).sum();
        let slow_middle_right: f64 = self.left_to_right().into_iter().filter(|&i| self.nodes[i].zone == 2 && self.nodes[i].rank > self.nodes[self.median.0].rank).map(|i| self.nodes[i].data).sum();
        println!("Left slow sum:{}, slow middle left sum:{}, slow middle right sum:{}, right slow sum:{}",left_slow_sum,slow_middle_left,slow_middle_right,right_slow_sum);
        println!("Internal sums:{:?}",self.sums);
        println!("Zones:{:?}",self.zones);
        println!("{:?}",self.left_to_right().into_iter().map(|i| self.nodes[i].data).collect::<Vec<f64>>());

        if
            left_slow_sum - self.sums[0] > 0.00001 ||
            right_slow_sum - self.sums[3] > 0.00001 ||
            slow_middle_left - self.sums[1] > 0.00001 ||
            slow_middle_right - self.sums[2] > 0.00001
        {
            panic!("Sum mismatch")
        }
...
mod rank_vector_tests {
    ...
    fn random_test() {
        ...
        for _ in 0..100 {
            eprintln!("Starting");
            let data = (0..10).map(|_| thread_rng().gen::<f64>()).collect::<Vec<f64>>();
            let mut draw_order = (0..10).collect::<Vec<usize>>();
            draw_order[..].shuffle(&mut thread_rng());
            let mut vector = RankVector::<Vec<Node>>::link(&data);
            vector.check_integrity();
            eprintln!("Linked");
            for i in draw_order {
                vector.pop(i);
                eprintln!("Popped");
