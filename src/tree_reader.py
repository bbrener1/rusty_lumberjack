...
class Forest:
    ...
    def plot_tree_summary(self,n=3):
        ...
        def recursive_axes(tree,limits=[0,1],n=3):
            ...
            for child in tree[1]:
                cw = recursive_axes(child,[x+child_width,y-(height)])
                child_dimensions.append([x+child_width,y-(height),cw])
                child_width += cw
            ...
            center_y = y - (height * .5)
            for cx,cy,cw in child_dimensions:
                child_center_x = cx + (cw/2)
                child_center_y = cy - (height/2)
                print(f"arrow debug,x:{center_x},y:{center_y},cx:{child_center_x},cy:{child_center_y}")
                arrow_canvas.plot([center_x,child_center_x],[center_y,child_center_y],transform=arrow_canvas.transAxes)
            ...
            if tree[0] < len(self.split_clusters) and tree[0] !=0:
                self.split_clusters[tree[0]].up_down_panel(ax,n)
            if tree[0] == 0:
        ...
        recursive_axes(self.likely_tree)
...
class NodeCluster:
    ...
    def changed_features(self,n=50,plot=True):
        ...
        feature_order = feature_order[degenerate_mask]
