...
class Node:
    ...
    def node_counts(self):
        node_counts = self.forest.output[self.samples].X
        return node_counts
    ...
    def feature_median(self,feature):
        values = self.forest.output[self.samples,feature].X
    ...
    def feature_mean(self,feature):
        values = self.forest.output[self.samples,feature]
    ...
    def dispersions(self):
        ...
        matrix = self.forest.output[self.samples].X
    ...
    def additive_gains(self):
        ...
        if self.parent is not None:
            ...
            parent_medians = np.zeros(self.forest.output.shape[1])
    ...
    def sample_names(self):
        return [self.forest.input.obs[i] for i in self.samples]
    ...
    def sorted_node_counts(self):
        ...
        try:
            sort_order = np.argsort(self.forest.input[self.samples][:,self.feature()])
        except:
...
class Braid:
    ...
    def braid_scores(self):
        scores = np.zeros(self.node.forest.output.shape[0])
...
class Tree:
    ...
    def tree_movie_frame(self,location,level=0,sorted=True,previous_frame=None,split_lines=True):
        ...
        if split_lines:
            for split in node_splits[:-1]:
                plt.plot([0,self.forest.output.shape[1]-1],[split,split],color='w')
    ...
    def plot_leaf_counts(self):
        ...
        heatmap = np.zeros((total_samples,self.forest.output.shape[1]))
        running_samples = 0
        for leaf in leaves:
...
########################################################################
    ...
    def input_features(self):
        return self.input.var_names

    def output_features(self):
        return self.output.var_names

    def node_sample_encoding(self,nodes):
        encoding = np.zeros((self.output.shape[0],len(nodes)),dtype=bool)
    ...
    def node_sister_encoding(self,nodes):
        encoding = np.zeros((self.output.shape[0],len(nodes)),dtype=int)
        for i,node in enumerate(nodes):
            for sample in node.samples:
                encoding[sample,i] = 1
...
########################################################################
    ...
    def absolute_gain_matrix(self,nodes):
        gains = np.zeros((self.output.shape[1],len(nodes)))
        for i,node in enumerate(nodes):
    ...
    def node_matrix(self,nodes):
        predictions = np.zeros((len(nodes),self.output.shape[1]))
        for i,node in enumerate(nodes):
            predictions[i] = node.medians()
        return predictions

    def weight_matrix(self,nodes):
        weights = np.zeros((len(nodes),self.output.shape[1]))
        for i,node in enumerate(nodes):
...
########################################################################
    ...
    def __init__(self,trees,input,output,test=None,split_labels=None,cache=False):
    ...
    def load(location, prefix="/run", ifh="/run.ifh",ofh='run.ofh',clusters='run.cluster',input="input.counts",output="output.counts",test="test.counts"):
        ...
        # input = AnnData(np.loadtxt(location+input),var={f:i for i,f in enumerate(ifh)})
        # output = AnnData(np.loadtxt(location+output),var={f:i for i,f in enumerate(ofh)})

        input = AnnData(np.loadtxt(location+input))
        input.var_names = ifh
        output = AnnData(np.loadtxt(location+output))
        output.var_names = ofh
        ...
        first_forest = Forest([],input=input,output=output,test=test,split_labels=split_labels)
    ...
    def add_annotated_feature(self,feature_values,feature_name=None):

        if feature_name is None:
            feature_name = str(self.output.obs.shape[1])

        self.output.obs[feature_name] = feature_values
...
########################################################################
    ...
    def set_sample_labels(self,sample_labels):
        ...
        for i,cluster in enumerate(one_hot):
            self.add_annotated_feature(cluster,feature_name=f"sample_cluster_{i}")
