...
from tree_reader_sc import Node as TreeReaderNode
...
class IHMM():
    def __init__(self,forest,alpha=1,beta=1,gamma=1,beta_e=None,start_states=20,inf_check=False,p=None,hierarchal=True):
        ...
        for node in self.live_nodes:
            # node_medians = node.medians()
            node_local_gains = node.local_gains()
            node_local_gains = np.log2(np.abs(node_local_gains) + 1) * np.sign(node_local_gains)
            # local_gains = node.local_gains
            # local_gains = node.medians
            self.node_features[node.index] = node_local_gains
        ...
        print(f"Dimensions:{self.node_features.shape}")
        print(f"Live nodes:{len(self.live_nodes)}")
    ...
    def update_states(self):
        ...
        self.state_log_odds_given_features = self.compute_state_log_odds_given_features(self.node_features,self.live_mask)
    ...
    def estimate_node_means(node_mask,node_summary):
        feature_means = np.mean(node_summary['node_features'][node_mask],axis=0)
        return (feature_means , np.sum(node_mask))

    def estimate_node_covariance_precision(node_mask,node_summary,prior=None,feature_means=None):

        nodes = np.sum(node_mask)
        features = node_summary['node_features'].shape[1]
        ...
        means = np.sum(node_features[node_mask],axis=0) / (nodes + 1)
        centered = node_features[node_mask] - np.tile(means,(np.sum(node_mask),1))

        covariance_estimate = ( np.dot(centered.T,centered) + (prior_matrix * prior_power) ) / (nodes + prior_power)

        # print(covariance_estimate[:10,:10])
        # print(np.cov(node_features[node_mask].T))
        # print(np.cov(node_features[node_mask].T) * nodes)
        # print(nodes + prior_power)
    ...
    def compute_data_properties(self):
        ...
        print("Computed data properties:")
        print(f"Means:{self.data_means.shape},{self.data_means[:10]}")
        print(f"C/V:{(self.data_covariance.shape,self.data_precision.shape)},{(self.data_precision[:10,:10])}")
    ...
    def compute_state_log_odds_given_features(self,node_features,live_mask):
        ...
        for i,component in enumerate(self.components[1:]):
            ...
            feature_log_odds[i][live_mask] = np.array(self.pool.map(Component.node_log_likelihood_async,zip(repeat(component),node_features[live_mask])))
    ...
    def sample_states(self,live_mask,state_log_odds):
        ...
        #     draw_index_async_handle.append(self.pool.apply_async(self.log_sampling,(odds)))
        ...
        draw_index = [IHMM.log_sampling(odds)+1 for odds in state_log_odds[:,live_mask].T]
...
class Component():
    ...
    def estimate_paramters(self,prior_means,prior_mean_precision,prior_covariance,prior_precision,prior_power):
        ...
        estimated_covariance,estimated_precision = IHMM.estimate_node_covariance_precision(self.mask,self.node_summary)
    ...
    def node_log_likelihood_async(task):
        component,features = task
        component.node_log_likelihood(features)

    def node_log_likelihood(self,features):

        # print("loglikedebug")
        # print(features.shape)
        # print(node_feature_mask.shape)

        centered = features - self.means

        precision_fit = np.dot(np.dot(centered,self.precision),centered)
        log_likelihood = self.covariance_log_determinant + precision_fit + np.log2(2*np.pi)
